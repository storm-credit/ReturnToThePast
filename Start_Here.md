# START HERE — 《나는 과거로 간다》 재설계판

Status: ACTIVE DESIGN ENTRY POINT  
...
- 장르: 다크 판타지 시간여행 음모 스릴러 + 정치 판타지
- 방향: A+C Hybrid
- 주인공: 죽으면 되돌아오는 회귀자가 아니라 여러 연대로 직접 파견되는 시간요원
- 세계: 수도·왕좌·성당·마탑·왕실 연대기가 결합된 왕국 전체가 시간 장치
- 핵심 엔진: 임무 성공 → 미래 변형 → 달라진 본부와 관계 → 이전 성공이 만든 새 재앙
...
기존 `Drafts/`, `outline/`, `lore_bible/`와 `codex/orchestra-setting-sync` 브랜치는 새 정본이 아니다. 아이디어 후보 자료로만 사용하며, 개별 심사 없이 승계·수정·병합하지 않는다.

## 집필 금지

작가가 아래 문장을 명시적으로 선언하기 전에는 신규 원고 작성, 기존 원고 재작성, 자동 확장과 문체 수술을 하지 않는다.
