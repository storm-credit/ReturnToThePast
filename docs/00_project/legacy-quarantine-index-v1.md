...
6. 관련 Domain Bible
7. 관련 Story Architecture
8. 관련 Context Pack / Craft Manifest / Harness

Deep Design 전체 상태는 `docs/00_project/PROJECT_COMPLETION_SCORECARD_20260820.md`를 함께 확인한다.
...
- `Ending_Scenarios.md` — 구 172회차/환생/관찰자 엔딩 자료
- `Lore_Bible_Master_Index.md` — 구 《나는 과거로 간다》 회귀·코스믹호러 설정집 인덱스
- `02_SESSION_SUMMARY.md` — 비어 있는 구 세션 인계 방식
...
- 세라핌 수도·구 회귀 설정의 장소명을 칼레온 정본 지명으로 자동 사용
...
Legacy 문서에 `정사`, `Final`, `절대 기준`이라고 적혀 있어도 현재 권한을 갖지 않는다.
...
- `outline/`, `Drafts/`, 이 문서 §3의 루트 파일은 정답 후보에서 제외한다.
