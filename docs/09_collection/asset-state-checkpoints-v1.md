...
| 초대왕의 무관 | 단독 승인권 후보 | 공동승인·감사 협약의 법적 증거 | **녹여 분산인장 제작. 용해 과정 전체를 공개 기록으로 남겨 물질은 사라지되 증거는 남는다** (DEC-020) |
...
| R05 | 존재하지 않는 도시의 지도(백지도) | · | · | L | L | · | · | L | · | · | L | · | · | L | · | S |
...
| R05 | V3 | 리아 세른 확보·휴대 | E075 현장조사 준비 자리에서 첫 제시 | L — 이름 미기입 상태. V4 공동주장 이전의 단독 취급 | E075, V03-3D, DEC-028 |
...
| 4 | ~~R10 최종상태~~ | ~~`CHK`·`MAL`은 "녹여 분산인장 제작", `RE`·`WSC`는 "비활성 공공증거 보존"~~ | **해소 (DEC-020)** — 녹이되 용해 과정을 공개 기록으로 남긴다. 물질은 사라지고 그 기록이 증거가 된다 |
...
- ~~R03 개혁가의 절검 파괴 시점 충돌~~ → **해소 (DEC-020): V15 E366.** `major-assets-ledger` R03을 수정했다.
- ~~R10 초대왕의 무관 최종상태 충돌~~ → **해소 (DEC-020): 녹이되 용해 과정을 공개 기록으로 남긴다.** V15 칸은 `물질 없음 / 공개 용해기록 보존`으로 읽는다. 4개 문서를 통일했다.
